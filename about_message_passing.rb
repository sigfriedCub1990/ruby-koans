...
class AboutMessagePassing < Neo::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    #
    # You can use __send__ if the name `send` clashes with one
    # already defined in the class. [This is literally from the Ruby docs :)]
  end
  ...
  # NOTE: Both obj.msg and obj.send(:msg) sends the message named :msg to
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    #
    # We can define some behavior if the method we're calling is not part of the class.
    # Hence "intercepting" all the unknown messages.
    #
    # NOTE:
    #
  ...
  def test_all_messages_are_caught
    ...
    message_builder = lambda { |method_name, args|
      "Someone called #{method_name} with <#{args.join(', ')}>"
    }

    assert_equal message_builder.call('foobar', []), catcher.foobar
    assert_equal message_builder.call('foobaz', [1]), catcher.foobaz(1)
    assert_equal message_builder.call('sum', (1..6).to_a), catcher.sum(1, 2, 3, 4, 5, 6)
  end
