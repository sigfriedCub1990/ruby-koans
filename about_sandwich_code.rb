...
class AboutSandwichCode < Neo::Koan
  def count_lines(file_name)
    file = File.open(file_name) # Kernel#open is risky
    ### Meat
    count = 0
    ...
    file&.close # Safe navigation operator, previously file.close if file
  end
  ...
  def find_line2(file_name, regex)
    file_sandwich(file_name) do |file|
      while (line = file.gets)
